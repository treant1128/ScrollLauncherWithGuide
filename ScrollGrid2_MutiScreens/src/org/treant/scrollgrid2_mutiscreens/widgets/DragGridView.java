...
//	}
	...
	private void onDrag(int x, int y){
		...
		if((x>=(moveNum+1)*(Configure.screenWidth-30*Configure.screenDensity)||   //移动坐标event。getX（）超过300(距离右边界20像素处的边缘)
	...
	private void onDrop(int x, int y){
		...
		//我想把上面那个判断注释掉了   因为被setVisibility(View.INVISIBLE)后pointToPosition检测为INVALID_POSITION了
			dropPosition=holdPosition; Log.i("到过", "到过"+holdPosition);
		}
		if(moveNum!=0){  //换页发生了
		...
		emerge.setAnimationListener(new AnimationListenerImpl(){
			...
			public void onAnimationEnd(Animation animation) {
				// TODO Auto-generated method stub
				super.onAnimationEnd(animation);
				((DragAdapter)getAdapter()).exchangePosition(dragPosition, dropPosition);Log.i("位置", dragPosition+"="+dropPosition);
			}
		});
	}
